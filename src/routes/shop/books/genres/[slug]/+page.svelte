<script>
    ...
    export let data
    const {name, bookCount, books, genres, featured} = data.books
    ...
    let baseURL = `/shop/books/genres/${slug}/`
    ...
    const metadata = {
            title: `Genre: ${name} ${thisPage}| Alkebu-Lan Images`,
            description: `Books in the genre ${name}`,
	    	image: metaImg,
            imageAlt: 'bookshelf',
            url: metaURL,
        }    
...
<section class="page-header" style="background-image: url({urlFor(settings.banner).width(1920).height(300).auto('format').url()});">
    <div class="container">
        <h2><small>Genre:</small><br>{name}</h2>
